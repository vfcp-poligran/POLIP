  ...
  cargarCursos() {
    ...
    this.cursosDisponibles = Object.entries(courseStates)
      .map(([nombreCurso, state]) => {
        // Usar el nombreCurso (clave del courseStates) para verificar el archivo
        ...
        return {
          nombre: state.metadata?.nombre || nombreCurso,                    // Nombre completo
          nombreAbreviado: state.metadata?.nombreAbreviado || '',           // Código abreviado
          codigo: codigoUnico,                                              // Código interno único
          codigoBase: state.metadata?.codigo || '',                         // Código base sin timestamp
